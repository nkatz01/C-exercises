    ...
    class MinimumBribe
    {
        ...
        static void minimumBribes(int[] q)
        {
            ...
            for (var j = 0; j < q.Length; j++)
            {
                ...
                if (q[j] <= j + 1)
                {
                    if (numbersFound.Count() > 0)//&&  q[j] > Enumerable.Range(numbersFound.Min(), numbersFound.Max()).Except(numbersFound).First()
                    {
                        //   if (q[j] > qOrdered.First(el => !numbersFound.Contains(el)))
                        //  if (q[j] > qOrdered.First(el => !numbersFound.Any(p => p.Equals(el))))
                        //if (q[j] > qOrdered.First(el =>  numbersFound.IndexOf(el).Equals(-1)))
                        //   if (q[j] > qOrdered.Where(el => numbersFound.IndexOf(el).Equals(-1)).First())
                        

                                 if (q[j] > qOrdered.Except( numbersFound).First())
                                // if (q[j] > qOrdered.ToList().First(el => !numbersFound.Contains(el)))
                                //  if (numbersFound.Count() > 0 && Enumerable.Range(1, q[j] - 1).Except(numbersFound).Count() > 0)//&&  q[j] > Enumerable.Range(numbersFound.Min(), numbersFound.Max()).Except(numbersFound).First()
