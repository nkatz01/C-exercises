            ...
            foreach (var matchday in rounds)//object that includes matches array
            {
                ...
                foreach (var mathcArr in matchesObj)
                {
                    ...
                    foreach (var entry in mathcArr.Value)
                    {
                        ...
                        goals += teamOneAndTwo.ElementAt(0).Equals(teamKey) ? Int32.Parse(entry.SelectToken("score1").ToString()) : teamOneAndTwo.ElementAt(1).Equals(teamKey) ? Int32.Parse(entry.SelectToken("score2").ToString()) : 0;
