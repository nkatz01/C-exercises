...
namespace CodeSignal
{
    class Program
    {
        static void Main(string[] args)
        {
          ...
          // Console.WriteLine( flr("LFRFRFR"));//(Test with: RF -> 3,FLLF -> 0,LFRFRFR -> 1)//program that returns minimum number of commands (steps and head turns) for a robat to retrun to original starting point on an (x,y) plain. 
        ...
        var striplower = directions.Where(i => Char.IsUpper(i));
            ...
            int minCommands = 0;
            minCommands = Math.Abs(coordinates.Item1 + coordinates.Item2);
